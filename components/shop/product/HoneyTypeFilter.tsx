...
export function HoneyTypeFilter() {
  ...
  const toggle = useCallback(
    (value: string) => {
      ...
      if (params.get("tur") === value) {
        params.delete("tur");
      } else {
        params.set("tur", value);
      }
      params.delete("sayfa");
        ...
        return (
          <button
            ...
            className={`
              ...
              ${isActive
                ? `${t.activeBg} ${t.activeBorder} text-white shadow-md scale-[1.04]`
                : `${t.bg} ${t.border} ${t.text} hover:scale-[1.02] hover:shadow-sm`
              }
            `}
          >
            <span className={isActive ? "text-white" : ""}>{t.icon}</span>
